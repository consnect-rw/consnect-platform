...
import { AdminOfferCard } from "@/components/cards/AdminOfferCard";
import { Loader2, FileText } from "lucide-react";

interface AdminOffersContainerProps {
     offers: TAdminOfferCard[];
     isLoading?: boolean;
     emptyMessage?: string;
}

export const AdminOffersContainer = ({ 
     offers, 
     isLoading = false,
     emptyMessage = "No offers found" 
}: AdminOffersContainerProps) => {
     if (isLoading) {
          return (
     ...
     return (
          <div className="grid gap-4 sm:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3">
               {offers.map((offer) => (
                    <AdminOfferCard key={`admin-offer-${offer.id}`} offer={offer} />
               ))}
          </div>
     );
};
